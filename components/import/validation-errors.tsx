...
export default function ValidationErrors({ batchId, errors }: { batchId: string | null; errors: ErrorCsvRow[] }) {
  return <Panel title="Validation Errors" meta={batchId ? `Batch ${batchId}` : '최근 오류 없음'}>
    {batchId && <div className="section-heading import-errors-heading"><span className="muted">가장 최근 오류 또는 경고가 있는 batch의 행 단위 결과입니다.</span><a className="button ghost" href={`/api/admin/import/error-csv?batchId=${encodeURIComponent(batchId)}`}>오류 CSV 다운로드</a></div>}
    <DataTable columns={[
